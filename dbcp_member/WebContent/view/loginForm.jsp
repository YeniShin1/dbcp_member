...
<%-- 세션에 값이 있다면 현재 로그인 상태임, 세션에 값이 없는 경우 비 로그인 상태이므로 로그인 폼 보여주기 --%>
<%
	//세션 여부 확인하기
...
%>
	<script>
		var name = '<%=vo.getName()%>';
		//3번 메뉴 없애기(header.jsp에서 id가 3번) ***
		var $menu = $('#3').detach(); 
		
		var $tag = '<span class = "navbar-text">'+name+' 님 반갑습니다.</span>' //navbar(부트스트랩에서 글자 색 가져오기 위함)
				+'&nbsp;&nbsp;'
				+'<button type="button" id="logout" class="btn btn-primary">로그아웃</button>';
		$("#navbarCollapse").append($tag); //지금까지 만든 것 append시켜주기***
	</script>
<% 
	}
%>
